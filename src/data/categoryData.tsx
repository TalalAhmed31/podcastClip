const dummyCategories = [
    {
      id: '1',
      name: 'Comedy',
      imageUrl: 'https://i.scdn.co/image/ab6765630000ba8a416d8e72329e9bb47d0477a2', // Replace with actual image
    },
    {
      id: '2',
      name: 'Technology',
      imageUrl: 'https://i.pinimg.com/474x/41/23/05/41230590a0ff35dd78f8856bd993affb.jpg',
    },
    {
      id: '3',
      name: 'News',
      imageUrl: 'https://t3.ftcdn.net/jpg/03/27/55/60/360_F_327556002_99c7QmZmwocLwF7ywQ68ChZaBry1DbtD.jpg',
    },
    {
      id: '4',
      name: 'Sports',
      imageUrl: 'https://www.shutterstock.com/image-photo/successful-football-soccer-basketball-baseball-600nw-1034455687.jpg',
    },
  ];

export default dummyCategories;
  